...
- **Stage 1 — a short, optional, form-like breadth pass.** Five pointed, collective-facing prompts plus a newsletter field. Captured even if the participant never starts stage 2. ~2 minutes, rough notes, every field skippable.
...
Five prompts, **all optional**, presented as quick written notes ("a sentence is plenty; rough notes, not essays; skip anything you want"), with a visible "skip to the conversation" affordance so a fully-skipped stage 1 is one click.
...
0. **Ordering:** `welcome` → `form` (stage 1) → `conversation` (stage 2). The time-budget / open-invitation question is **not** part of the form; it stays where it is today — the first turn of the stage-2 conversation. Stage 1 is purely the five prompts + newsletter.
...
- Prompt: a stage-2 smoke run with a populated map confirming it opens on a stage-1 thread without re-asking it verbatim, and a run with an empty map confirming fallback to the current opener.
